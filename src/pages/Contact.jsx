            ...
            <motion.div initial={{ opacity: 0, x: -40 }} animate={formInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}
              ...
              <form action="mailto:projects.abdulhannan@gmail.com" method="POST" encType="text/plain" className="space-y-5">
                <input type="text" name="name" placeholder="Your Name"
                  className="w-full bg-[var(--glass-bg)] border border-light rounded-xl px-5 py-3.5 text-sm text-body placeholder:text-dim outline-none focus:border-brand/50 transition-all duration-300" />
                <input type="email" name="email" placeholder="Your Email"
                  className="w-full bg-[var(--glass-bg)] border border-light rounded-xl px-5 py-3.5 text-sm text-body placeholder:text-dim outline-none focus:border-brand/50 transition-all duration-300" />
                <input type="text" name="subject" placeholder="Subject"
                  className="w-full bg-[var(--glass-bg)] border border-light rounded-xl px-5 py-3.5 text-sm text-body placeholder:text-dim outline-none focus:border-brand/50 transition-all duration-300" />
                <textarea name="message" rows={5} placeholder="Tell me about your project..."
                  className="w-full bg-[var(--glass-bg)] border border-light rounded-xl px-5 py-3.5 text-sm text-body placeholder:text-dim outline-none focus:border-brand/50 transition-all duration-300 resize-none" />
                <button type="submit"
                  className="w-full px-8 py-3.5 bg-brand text-[var(--text-on-brand)] rounded-full font-medium text-sm hover:shadow-[0_0_30px_rgba(3,38,252,0.4)] transition-all duration-300">
                  Send Message
                </button>
              </form>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} animate={formInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} className="space-y-5">
              <div className="glass rounded-3xl p-8 lg:p-10">
                ...
                <div className="space-y-4">
                  <a href="mailto:projects.abdulhannan@gmail.com"
                    className="flex items-center gap-4 p-4 rounded-xl glass glass-hover transition-all duration-300 group">
                    <div className="w-10 h-10 rounded-lg bg-brand/10 flex items-center justify-center text-brand transition-all duration-300">
              ...
              <div className="glass rounded-3xl p-8 lg:p-10 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-secondary text-sm italic mb-4">&ldquo;AI accelerates my workflow. It does not replace my thinking.&rdquo;</p>
                  <div className="flex flex-wrap gap-2">
                    {['Research', 'Planning', 'Architecture', 'Docs', 'Automation'].map((item) => (
                      <span key={item} className="px-3 py-1 text-xs border border-brand/20 text-brand/70 rounded-full">{item}</span>
                    ))}
                  </div>
                </div>
              </div>
